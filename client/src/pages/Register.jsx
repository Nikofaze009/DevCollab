...
const Register = () => {
  ...
  const focusIn = (e) => { e.target.style.borderColor = '#111'; };
  const focusOut = (e) => { e.target.style.borderColor = 'rgba(0,0,0,0.1)'; };
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8rem 2rem 4rem' }}>
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '3.2rem' }}>
          ...
          <h1 style={{ fontSize: '3.2rem', fontWeight: 900, letterSpacing: '-0.04em', color: '#111', marginBottom: '0.8rem' }}>create account.</h1>
          <p style={{ color: 'rgba(0,0,0,0.5)', fontSize: '1.4rem', fontWeight: 500 }}>Join DevCollab today. It's free.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '2.4rem' }}>
          <div>
            ...
            <input type="text" value={username} onChange={e => setUsername(e.target.value)} required style={inputStyle} placeholder="johndoe" onFocus={focusIn} onBlur={focusOut} />
          </div>
          <div>
            <label style={labelStyle}>Email</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} required style={inputStyle} placeholder="you@example.com" onFocus={focusIn} onBlur={focusOut} />
          </div>
          <div>
            <label style={labelStyle}>Password</label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} required style={inputStyle} placeholder="••••••••" onFocus={focusIn} onBlur={focusOut} />
          </div>
          <button
        ...
        <p style={{ marginTop: '3.2rem', textAlign: 'center', fontSize: '1.4rem', color: 'rgba(0,0,0,0.5)', fontWeight: 500 }}>
          ...
          <Link to="/login" style={{ color: '#111', textDecoration: 'none', fontWeight: 800, borderBottom: '2px solid #00c547' }}>Sign in</Link>
        </p>
      </div>
    </div>
  );
